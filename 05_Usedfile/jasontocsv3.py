...
path_dir = 'C:\\Users\\dlwlt\\Desktop\\C041_평가'
...
for j in range(len(file_list)):
    persons = 0
    os.mkdir('C:\\Users\\dlwlt\\Desktop\\C041_평가CSV\\' + file_list[j].split('_')[0]+ file_list[j].split('_')[1]+ file_list[j].split('_')[2]+ file_list[j].split('_')[3]+ file_list[j].split('_')[4]+ file_list[j].split('_')[5])  # ./AI-Hub_CSV2/1-1 ... ./AI-Hub_CSV2/706-1
    ...
    for m in range(persons):  # 0, 1, 2, ..., persons-1
        ...
        line = np.array(line)
        np.savetxt('C:\\Users\\dlwlt\\Desktop\\C041_평가CSV\\' + file_list[j].split('_')[0] + file_list[j].split('_')[1]+ file_list[j].split('_')[2]+ file_list[j].split('_')[3]+ file_list[j].split('_')[4]+ file_list[j].split('_')[5]+ '/' + '{:04d}.csv'.format(m + 1), line, fmt="%s",
                   delimiter=",")